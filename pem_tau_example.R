...
longdata <- longdata %>%
  ...
  mutate(t_id = seq(n())) %>%
  ungroup()
t_obs <- c(test_tau[test_tau <= max(longdata$os_months)][-1], max(longdata$os_months))
...
gen_inits <- function(M) {
  function() 
    list(
      ...
      c_raw = abs(rnorm(1)),
      r_raw = abs(rnorm(1)),
      baseline = rgamma(n = length(t_dur), shape = 0.01, scale = 0.1)
    )
}
#----Run Stan-------#
nChain <- 1
...
gen_inits <-  function(M) {
  function() 
    list(
      ...
      tau_s_bg_raw = 0.1*abs(rnorm(1)),
      tau_bg_raw = abs(rnorm(M)),
      sigma_baseline = abs(rcauchy(n = 1, 0 , 0.005)),
      baseline = abs(rnorm(n = length(t_dur), 0, 0.001))
    )
}
#-----Run Stan-------#
nChain <- 2
stanfile <- 'pem_bg_up_sigma.stan'
rstan_options(auto_write = TRUE)
simulated_fit <- stan(stanfile,
                      data = gen_stan_data(longdata),
                      init = gen_inits(M=2),
