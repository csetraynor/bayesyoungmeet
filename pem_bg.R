...
set.seed(342)
test_n = 100
test_tau = c(seq(0, 900, length.out = test_n))
...
gen_inits <- function() {
  list(
    ...
    c_raw = abs(rnorm(1)),
    r_raw = abs(rnorm(1)),
    baseline = rgamma(n = length(diff(tau)), shape = exp(3), scale = 0.1)
...
#----Convergence review -----#
print(simulated_fit)
pairs(simulated_fit, pars = c("lp__", "beta_bg_raw"), las = 1)
...
pp_beta_bg <- as.data.frame.array(rstan::extract(simulated_fit,pars = 'beta_bg', permuted = TRUE)$beta) 
